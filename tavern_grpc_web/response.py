...
from tavern_grpc_web.request import GRPCWebWrappedResult
...
def _normalize_status_list(spec: StatusSpec | None) -> list[str]:
    ...
    for item in items:
        ...
        if isinstance(item, bool):
            raise exceptions.GRPCServiceException(f"Unknown gRPC status code: {item!r}")
    ...
    def _validate_block(self, blockname: str, block: Mapping) -> None:
        ...
        if isinstance(expected_block, dict):
            ...
            if expected_block.get("$ext"):
                logger.warning("In %s block, $ext is deprecated; use verify_response_with", blockname)
    ...
    def verify(self, response: GRPCWebWrappedResult) -> Mapping:
        ...
        if "http_status_code" in self.expected:
            ...
            if exp_http != r.http_status_code:
                self._adderr("Expected http status code %s, got %s", exp_http, r.http_status_code)

        if "details" in self.expected:
            if self.expected["details"] != r.grpc_message:
                self._adderr("Expected grpc-message/details %r, got %r", self.expected["details"], r.grpc_message)

        saved = self._handle_body(r) or {}

        if self.errors:
            raise TestFailError(f"Test {self.name:s} failed verification:\n{self._str_errors():s}",
