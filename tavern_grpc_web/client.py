...
from tavern_grpc_web.codec import decode_grpc_web_body, encode_data_frame
...
def _split_service_method(full: str) -> tuple[str, str]:
    ...
    if not service or not method:
        raise exceptions.GRPCRequestException(f"Invalid service/method name: {full!r}")
...
class GRPCWebSession:
    def __init__(self, **kwargs: Any) -> None:
        ...
        if kwargs:
            raise exceptions.BadSchemaError(f"Unexpected keys in grpc_web configuration: {set(kwargs)!r}")

        self.base_url = (connect.get("base_url") or "").rstrip("/")
        if not self.base_url:
            raise exceptions.BadSchemaError("Please provide grpc_web.connect.base_url")
        ...
        if not proto_source and not proto_module:
            raise exceptions.BadSchemaError("In grpc_web.proto, provide at least one of: module or source")
        ...
        if proto_module:
            try:
                ...
                raise exceptions.GRPCServiceException("Failed to import generated protobuf/grpc modules") from e
    ...
    def call(
        ...
        if body is not None:
            try:
                ...
                raise exceptions.GRPCRequestException("Failed to build request from JSON payload (body/json)") from e
        ...
        try:
            ...
            raise exceptions.GRPCRequestException(f"HTTP error while calling service: {e}") from e
