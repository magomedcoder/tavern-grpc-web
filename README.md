...
External `grpc_web` backend plugin for Tavern.

The plugin allows Tavern to call unary RPC methods over HTTP using the gRPC-Web
wire format.
...
Connection settings are configured at the top level in the `grpc_web.connect`
block:
...
- `path_prefix`: Prefix inserted before `package.Service/Method` (defaults to
  `rpc`)
...
- `status`: Accepts canonical names (for example `"OK"`) and numeric codes (for
  example `0`); either form is valid.
- `details`: Expected value for the `grpc-message` trailer/header
...
If `grpc_web_response` is omitted, Tavern still performs the request but no
response assertions are made.
